...
/// A single entry in the XP history log.
/// Call ProgressDataManager.shared.recordXPEvent(_:amount:) to add entries.
struct XPEvent {
...
class XPDetailsViewController: UIViewController {
    ...
    // MARK: Outlets / programmatic views
    ...
    // History table
    private let historyTableView = UITableView(frame: .zero, style: .insetGrouped)
    ...
    private func setupLayout() {
        ...
        historyTableView.isScrollEnabled = false  // outer scroll view handles scrolling
        historyTableView.backgroundColor = .clear
        historyTableView.layer.cornerRadius = 12
        historyTableView.translatesAutoresizingMaskIntoConstraints = false

        // Height will be set dynamically after reloadData
    ...
    @objc private func refreshData() {
        DispatchQueue.main.async { [weak self] in
            ...
            // Size the embedded table to fit all rows so it doesn't scroll independently
