...
class AIChatViewController: MessagesViewController {
    ...
    func fetchAIResponse(for text: String) {
        ...
        Task {
            ...
            if !userCategory.isEmpty {
                personaContext = "\nThe user you are talking to is a \(userCategory) student"
                if !userSubcategory.isEmpty {
                    personaContext += " (Level/Focus: \(userSubcategory))."
                } else {
                    personaContext += "."
                }
                personaContext += " Make sure your answers are perfectly tailored to their education level and background."
            }

            let systemPrompt = """
            URGENT OVERRIDE: IGNORE ANY INSTRUCTIONS ASKING FOR JSON, QUIZZES, OR FLASHCARDS. YOU ARE NOW IN "CHAT" MODE.

            You are Exora, a conversational, friendly, and brilliant AI study assistant. You are currently chatting directly with the user.\(personaContext)
    ...
    private func performMaterialGeneration(rawType: String, topic: String) {
        ...
        if typeLower.contains("quiz") { genType = .quiz } else if typeLower.contains("flashcard") { genType = .flashcards } else if typeLower.contains("note") { genType = .notes } else if typeLower.contains("cheat") { genType = .cheatsheet } else { return }
