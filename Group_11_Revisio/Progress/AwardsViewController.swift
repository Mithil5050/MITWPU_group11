...
class AwardsViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    // ✅ Added the empty didTapShowAllButton() to fix the protocol error!
    extension AwardsViewController: MonthlyBadgeCellDelegate {
        func didTapMonthlyBadgeCard() {
            print("Image Tapped! Navigating to Monthly Challenge...")
            
            // Instantiating directly from the XIB file instead of a Storyboard
            let nextVC = MonthlyChallengeDetailViewController(nibName: "MonthlyChallengeDetailViewController", bundle: nil)
            self.navigationController?.pushViewController(nextVC, animated: true)
        }
            
        
        // Satisfies the protocol requirement without doing anything
